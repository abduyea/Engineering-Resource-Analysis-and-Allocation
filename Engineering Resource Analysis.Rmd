...
```{r}
# Total cost and total units produced
total_cost <- sum(monthly_data$total_cost)
total_units <- sum(monthly_data$total_classic_cars + monthly_data$total_motorcycles + monthly_data$total_planes + monthly_data$total_ships)
...
```
What is the Average Profit Per Product Line?
Using a selling price of $500 per unit, we calculate the average profit per unit by subtracting the production cost per unit from the selling price.
```{r}
